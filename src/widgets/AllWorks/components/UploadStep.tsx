...
export const UploadStep = (props: UploadStepProps) => {
  ...
  const imprintLabels = imprintsOptions.map((imprint) => imprint.label);
    ...
    headers: [
      {
        ...
        validate: (field: string | number | boolean) => imprintLabels.includes(`${field}`),
  ...
  const validateCsvFile = async (file: File) => {
    try {
      ...
      const contributorsNames = csvParseResult.data
        .map((row) => `${row.contribution1FirstName ?? ''} ${row.contribution1Surname ?? ''}`.trim())
        .filter((value) => value && value.length > 0);

      console.log('contributorsNames', contributorsNames);
      console.log('csvParseResult', csvParseResult);

      const promises = contributorsNames.map((name) => contributorService.getContributors(name));
