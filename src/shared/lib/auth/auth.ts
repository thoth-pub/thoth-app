...
async function refreshAccessToken(token: JWT): Promise<JWT> {
  ...
  try {
    ...
    const tokenEndpointResponse = await oidc.refreshTokenGrant(config, token.refreshToken as string);
 ...
 */
export async function buildLogoutUrl(idToken: string): Promise<{ url: string; state: string }> {
  const config = await oidc.discovery(
    new URL(process.env.ZITADEL_DOMAIN!),
    process.env.ZITADEL_CLIENT_ID!,
    process.env.ZITADEL_CLIENT_SECRET,
