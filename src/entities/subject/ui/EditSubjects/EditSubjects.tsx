...
import { type BaseEditSectionProps, convertOptionToString, IDs, SubjectTypes } from '@/src/shared';
...
const EditSubjects = (props: BaseEditSectionProps) => {
  ...
  return (
    <EditableContent
      ...
      preview={({ onEdit }) => (
        <Preview label={SUBJECTS.label} onEdit={onEdit} value={placeholder}>
          {placeholder && (
            <ul className="flex w-full flex-col gap-[var(--default-gap)]">
              {bicubSubjects.length > 0 && (
                <PreviewList subjects={bicubSubjects} title={SubjectTypes.enum.Bic} onDelete={deleteSubject} />
              )}
              {bisacSubjects.length > 0 && (
                <PreviewList subjects={bisacSubjects} title={SubjectTypes.enum.Bisac} onDelete={deleteSubject} />
              )}
              {customSubjects.length > 0 && (
                <PreviewList subjects={customSubjects} title={SubjectTypes.enum.Custom} onDelete={deleteSubject} />
              )}
              {keywordSubjects.length > 0 && (
                <PreviewList subjects={keywordSubjects} title={SubjectTypes.enum.Keyword} onDelete={deleteSubject} />
              )}
              {lccSubjects.length > 0 && (
                <PreviewList subjects={lccSubjects} title={SubjectTypes.enum.Lcc} onDelete={deleteSubject} />
              )}
              {themaSubjects.length > 0 && (
                <PreviewList subjects={themaSubjects} title={SubjectTypes.enum.Thema} onDelete={deleteSubject} />
              )}
            </ul>
          )}
        </Preview>
